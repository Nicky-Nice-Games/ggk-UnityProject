...
public class Driver : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        // Handle ground alignment if grounded
        if (isGrounded)
        {
            // Snap to ground with offset
            Vector3 groundSnapPosition = hit.point + hit.normal * suspensionOffset;
            transform.position = groundSnapPosition;

            // Align rotation with ground normal
            Quaternion groundRotation = Quaternion.FromToRotation(transform.up, hit.normal) * transform.rotation;
            transform.rotation = Quaternion.Slerp(transform.rotation, groundRotation, rotationAlignSpeed * Time.deltaTime);
        }
        // Adjust drag depending on grounded state
    ...
    /// <summary>
    /// Checks if the kart is grounded by casting a short ray downwards
    /// </summary>
    /// <returns></returns>
    public bool IsGrounded()
    {
        return Physics.Raycast(transform.position + Vector3.up * 0.1f, Vector3.down, groundDist, groundLayer);
    }
